...
`rirods:::remove_mock_files()` simplifies this task. This function is not exported and 
needs to be called with triple colons `:::` or sourced (e.g., using `devtools::load_all()`) and subsequently 
called.
...
Alternatively, one can use the function `use_irods_demo()`.
...
function: `rirods:::local_create_irods()`. This test fixture is almost the same as the
exported function `create_irods()` except that it uses an alternative directory
(default to a temporary directory) as the working directory thereby preventing 
cluttering of the package source files. The working directory can be restored to the 
