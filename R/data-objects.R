...
iput <- function(
  ...
  stop_irods_overwrite(overwrite, lpath)
...
# internal
iput_ <- function(x, offset, count, lpath, truncate, verbose) {
...
iget <- function(
  ...
  # local path
  path <- file.path(path, basename(lpath))

  # expand logical path to absolute logical path
  lpath <- get_absolute_lpath(lpath)

  # file size on irods
  file_size <- ils(path = lpath, stat = TRUE)[[2]][[2]]
  ...
  if (is.character(lpath) && tools::file_ext(lpath) != "rds") {
...
# internal
iget_ <- function(lpath, offset, count, verbose) {
